...
- 主要分为三部分：鼠标映射，键盘映射，摇杆映射
## 鼠标映射
## 键盘映射
 ### 整体思路：需要一个控件，位置处于其余控件的上层，便于接收各种事件(如按键事件，手势事件)，并进行相应地处理同时自身不消费事件，处理完毕后，传递给下层的控件；另外需要一个配置界面。
...
- DirectionKeyTouchListener类，轮盘点击事件监听，主要处理轮盘的拖动和缩放；
``` 
  @Override
...
``` 
- DirectionKeyHoverListener类，轮盘鼠标指针悬停监听，主要判断鼠标指针是否进入缩放范围；
``` 
  @Override
